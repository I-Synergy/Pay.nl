...
namespace PayNLFormsApp
{
    public partial class DebugForm : Form
    {
        ...
        {
            ClearDebug();
            var request = new PayNL.API.Transaction.GetService.Request();
            InitRequestDebug(request);
            await ClientService.PerformPostRequestAsync(request);
            DebugRawResponse(request);
            tbMain.Text = request.Response.ToString();
        ...
        {
            ClearDebug();
            var request = new PayNL.API.Transaction.GetLastTransactions.Request();
            InitRequestDebug(request);
            await ClientService.PerformPostRequestAsync(request);
            DebugRawResponse(request);
            tbMain.Text = request.Response.ToString();
        ...
        public async Task DeclineAsync(string transactionID)
        {
            try
            {
                ...
                {
                    var request = new PayNL.API.Transaction.Decline.Request
                    {
                        TransactionId = transactionID
                    };
        ...
        public async Task RefundAddAsync(string bankAccountName, string bankAccountNumber, string amount)
        {
            try
            {
                ...
                {
                    var request = new PayNL.API.Refund.Add.Request(numValue, bankAccountName, bankAccountNumber, "");
                    InitRequestDebug(request);
        ...
        public async Task TransactionRefundInfoAsync(string refundID)
        {
            try
            {
                ...
                var request = new PayNL.API.Refund.Info.Request(refundID);
                InitRequestDebug(request);
